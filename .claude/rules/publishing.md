...
- [ ] `CHANGELOG.md` has an entry for the new version with a clear description
...
- [ ] README explains OAuth app setup steps in Dynatrace (required for reviewers who don't have a tenant)
...
- README must include setup instructions (Dynatrace OAuth app creation is non-obvious)
...
Bump `version` in `package.json` and add a `CHANGELOG.md` entry before every publish.
