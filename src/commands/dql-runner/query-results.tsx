...
export default function QueryResultsView({
  dql,
  timeframe,
  onClose,
      ...
      actions={
        <ActionPanel>
          ...
          <Action
            ...
            icon={Icon.Clipboard}
            onAction={handleCopyJson}
          />
          <Action
            title="Copy DQL Query"
            icon={Icon.Clipboard}
            onAction={handleCopyDql}
          />
          {onClose && (
      ...
      <List.Section title={`Results (${data.records.length} records)`}>
        {data.records.map((record, index) => {
          ...
          const title = titleMapping
            ? String(recordObj[titleMapping.key] ?? "—")
            : `Record ${index + 1}`;
          const subtitle = subtitleMapping
            ? String(recordObj[subtitleMapping.key] ?? "")
            : undefined;
              ...
              actions={
                <ActionPanel>
                  <Action
                    ...
                    icon={Icon.Eye}
                    onAction={() => {
                      // In a real implementation, you'd push a detail view
                      // For now, just show in markdown
                      const detail = JSON.stringify(recordObj, null, 2);
                      showToast({
                        style: Toast.Style.Success,
